...
@Model.register("multitask_v2")
class MultiTaskModelV2(Model):
    """
    ...
    def __init__(
        ...
        heads: Dict[str, Head],
        *,
        multiple_heads_one_data_source: bool = False,
        desired_order_of_heads: List[str] = None,
    ...
    def forward(self, **kwargs) -> Dict[str, torch.Tensor]:  # type: ignore
        ...
        if len(set(kwargs["task"])) != 1:
            raise ValueError(
                "All items should be from the same task."
            )
