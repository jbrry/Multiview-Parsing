...
class MultiTaskTaggerHead(Head):
    ...
    def make_output_human_readable(
        ...
        arg_maxes = all_predictions.argmax(-1)
        np_maxes = arg_maxes.cpu().detach().numpy()

        all_tags = [] # list to keep batch elements
        for predictions in np_maxes:
            tags = [self.vocab.get_token_from_index(tag, namespace=self.task) for tag in predictions]
