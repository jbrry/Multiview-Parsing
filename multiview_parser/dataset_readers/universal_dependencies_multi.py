    ...
    for i in range(len(annotation)):
        ...
        if type(conllu_id) == tuple:
            if "-" in conllu_id:
                ...
                annotation[i]["id"] = None # conllu_id (not worried about elided ids for basic parsing.)
...
@DatasetReader.register("universal_dependencies_multi", exist_ok=True)
class UniversalDependenciesMultiDatasetReader(DatasetReader):
    """
    ...
    def __init__(
        self,
        multi_token_indexers: Dict[str, TokenIndexer] = None,
        mono_token_indexers: Dict[str, TokenIndexer] = None,
        mono_token_character_indexers: Dict[str, TokenIndexer] = None,
        mono_sentence_character_indexers: Dict[str, TokenIndexer] = None,
        tokenizer: Tokenizer = None,
        **kwargs,
    ) -> None:
        super().__init__(**kwargs)
        self._multi_token_indexers = multi_token_indexers or {"tokens": SingleIdTokenIndexer()}
        self._mono_token_indexers = mono_token_indexers or {"tokens": SingleIdTokenIndexer()}
        self._mono_token_character_indexers = mono_token_character_indexers  or {"token_characters": TokenCharactersIndexer()}
        self._mono_sentence_character_indexers = mono_sentence_character_indexers or {"sentence_characters": SingleIdTokenIndexer()}
    ...
    def _read(self, file_path: str):
        ...
        with open(file_path, "r") as conllu_file:
            ...
            for annotation in parse_incr(conllu_file):
                ...
                yield self.text_to_instance(ids, multiword_ids, multiword_forms, misc, conllu_metadata,
    ...
    def text_to_instance(
        self,  # type: ignore
        ids: List[str],
        multiword_ids: List[str],
        multiword_forms: List[str],
        misc: List[str],
        ...
        multi_text_field = TextField(tokens, self._multi_token_indexers)
        mono_text_field = TextField(tokens, self._mono_token_indexers)

        ##### Token-level character
        token_char_text_field = TextField(tokens, self._mono_token_character_indexers)
        
        ##### Sentence-level character
        # Splits text into characters (instead of words)
        sentence_char_tokenizer = CharacterTokenizer()
        sentence_char_tokens = sentence_char_tokenizer.tokenize(" ".join(words))

        # Represents each token (which is a character) as a single id from a vocabulary
        sentence_char_text_field = TextField(sentence_char_tokens, self._mono_sentence_character_indexers)
        
        fields["multi_words"] = multi_text_field
        fields["words"] = mono_text_field
        fields["token_characters"] = token_char_text_field
        fields["sentence_characters"] = sentence_char_text_field
        ...
        for name, field in zip(names, all_tags):
            if field:
                fields[name] = SequenceLabelField(field, mono_text_field, label_namespace=name)

        if dependencies is not None:
            # We don't want to expand the label namespace with an additional dummy token, so we'll
            # always give the 'ROOT_HEAD' token a label of 'root'.
            fields["head_tags"] = SequenceLabelField(
                [x[0] for x in dependencies], mono_text_field, label_namespace="head_tags"
            )
            fields["head_indices"] = SequenceLabelField(
                [x[1] for x in dependencies], mono_text_field, label_namespace="head_index_tags"
            )
