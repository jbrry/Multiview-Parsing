...
@TokenEmbedder.register("pretrained_transformer_dataset")
class PretrainedTransformerDatasetEmbedder(TokenEmbedder):
    """
    ...
    def forward(
        ...
        # dataset embeds are being added here (code taken from Machamp):
        # https://bitbucket.org/robvanderg/dataembs2/src/master/mtp/machamp/modules/pretrained_transformer_embedder.py
        if dataset_ids is not None and enc_dataset_embedding is not None:
            ...
            for sent_idx in range(len(wordpiece_sizes)):
                ...
                for word_idx in range(min(len(wordpiece_sizes[sent_idx]), len(dataset_ids[sent_idx]))):
                    # get number of wordpieces for this word and fill data_ids with this many pieces
                    for _ in range(wordpiece_sizes[sent_idx][word_idx]):
                        if piece_idx >= len(data_ids[sent_idx]):
                            continue
                        # match wordpiece level with word level
                        data_ids[sent_idx][piece_idx] = dataset_ids[sent_idx][word_idx]
                        piece_idx += 1
            
            dataset_embeds = enc_dataset_embedding(data_ids)
