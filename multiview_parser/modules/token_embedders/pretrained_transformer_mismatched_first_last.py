...
@TokenEmbedder.register("pretrained_transformer_mismatched_first_last")
class PretrainedTransformerMismatchedFirstLastEmbedder(TokenEmbedder):
    """
    ...
    def __init__(
        ...
        sub_token_mode: Optional[str] = "first_last",
    ) -> None:
        super().__init__()
    ...
    def forward(
        ...
        elif self.sub_token_mode == "first_last":
            # Select first sub-token embeddings from span embeddings
            # Shape: (batch_size, num_orig_tokens, embedding_size)
            first_embeddings = span_embeddings[:, :, 0, :]
