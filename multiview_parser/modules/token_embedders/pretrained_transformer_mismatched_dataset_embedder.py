...
from multiview_parser.modules import PretrainedTransformerDatasetEmbedder


@TokenEmbedder.register("pretrained_transformer_mismatched_dataset")
class PretrainedTransformerMismatchedDatasetEmbedder(TokenEmbedder):
    """
    ...
    def __init__(
        ...
        self._matched_embedder = PretrainedTransformerDatasetEmbedder(
            model_name,
    ...
    def forward(
        ...
        # dataset embedding code taken from Machamp: https://bitbucket.org/robvanderg/dataembs2/src/master/mtp/machamp/modules/pretrained_transformer_mismatched_embedder.py
        if dataset_ids is not None and enc_dataset_embedding is not None:
            wordpiece_sizes = []
            for sent_idx in range(len(offsets)):
                ...
                for word_idx in range(len(offsets[sent_idx])):
                    if offsets[sent_idx][word_idx][0] == 0:
                        continue
                    # get difference between the end and the start of the offset to determine word length (+1 as wordpieces must be at least length 1)
            ...
            embeddings = self._matched_embedder(
                ...
                wordpiece_sizes=wordpiece_sizes, enc_dataset_embedding=enc_dataset_embedding)
        else:
            embeddings = self._matched_embedder(
                token_ids, wordpiece_mask, type_ids=type_ids, segment_concat_mask=segment_concat_mask, dataset_ids=dataset_ids
            )
